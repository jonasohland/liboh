...
namespace o::io {
            ...
            struct recursor {
                ...
                void operator()(boost::system::error_code ec) {
                    ...
                    if (!handler(ec)) {
                        ...
                        if (duration.count()) tm_->expires_after(duration);
    ...
    struct metro {
        ...
        using repeat_callback = std::function<bool(boost::system::error_code)>;

        explicit metro(boost::asio::io_context& ctx, Duration duration)
            : duration_(duration), timer_(ctx) {}

        template <typename Duration>
        void start(Duration&& d, repeat_callback&&) {
            timer_.async_wait(repeat_callback);
        }

        void add_cb(repeat_callback&& cb) {
            cbs_.emplace_back(std::forward<repeat_callback>(cb));
        }

      private:
        void timer_cb(boost::system::error_code ec) {

            bool erase = false;

            for (OHLANO_IT_RANGE(it, cbs_)) {
                bool erase_this = (*it)(ec);

                erase = erase && erase_this;
            }
        }
